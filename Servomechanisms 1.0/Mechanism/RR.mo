...
class RR "RR Mechanism"
  ...
  inner Modelica.Mechanics.MultiBody.World world(g = g) annotation(Placement(transformation(origin = {-81.2721,-80.9187}, extent = {{-10,-10},{10,10}})));
  //Parameters 1
  ...
  //Components
  Servomechanisms.Mechanism.RLink rotational2(shape = shape2, n = n2, r = r2, r_CM = r_CM2, m = m2, shapeLength = shapeLength2, shapeWidth = shapeWidth2, shapeHeight = shapeHeight2, shapeType = shapeType2, extra = extra2, I_11 = I_112, I_22 = I_222, I_33 = I_332, I_21 = I_212, I_31 = I_312, I_32 = I_322, boxLength = boxLength2, boxWidth = boxWidth2, boxHeight = boxHeight2, cylinderLength = cylinderLength2, cylinderDiameter = cylinderDiameter2, density = density2) annotation(Placement(transformation(origin = {54.5785,4.15691}, extent = {{-27.2482,-27.2482},{27.2482,27.2482}})));
  Servomechanisms.Mechanism.RLink rotational1(shape = shape1, n = n1, r = r1, r_CM = r_CM1, m = m1, shapeLength = shapeLength1, shapeWidth = shapeWidth1, shapeHeight = shapeHeight1, shapeType = shapeType1, extra = extra1, I_11 = I_111, I_22 = I_221, I_33 = I_331, I_21 = I_211, I_31 = I_311, I_32 = I_321, boxLength = boxLength1, boxWidth = boxWidth1, boxHeight = boxHeight1, cylinderLength = cylinderLength1, cylinderDiameter = cylinderDiameter1, density = density1) annotation(Placement(transformation(origin = {-23.5012,4.29742}, extent = {{-27.2482,-27.2482},{27.2482,27.2482}})));
  Modelica.Mechanics.MultiBody.Parts.Fixed fixed1 annotation(Placement(transformation(origin = {-82.904,3.27869}, extent = {{-10,-10},{10,10}})));
  Modelica.Mechanics.Rotational.Interfaces.Flange_a flange_rotational1 annotation(Placement(transformation(origin = {-45.4333,-44.9649}, extent = {{-10,-10},{10,10}}), iconTransformation(origin = {-97.8923,-10.7729}, extent = {{-10,-10},{10,10}})));
  Modelica.Mechanics.Rotational.Interfaces.Flange_a flange_rotational2 annotation(Placement(transformation(origin = {34.6604,-48.7119}, extent = {{-10,-10},{10,10}}), iconTransformation(origin = {-23.4192,59.0164}, extent = {{-10,-10},{10,10}})));
equation
